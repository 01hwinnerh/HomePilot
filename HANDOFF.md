...
- 当前分支预期：`feat/storefront-auth`（由用户自行确认，不要自行执行 Git 命令）。
- 当前未提交模块：`@homepilot/auth-client`；其 test（2/2）、TypeScript build、lint 均已通过用户验收。
- 下一小模块：仅实现 Storefront 的 Zustand 内存会话状态及单元测试；**先向用户说明选型、备选方案和测试行为，获得确认后再编码**。
